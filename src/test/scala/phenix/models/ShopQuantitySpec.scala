...
class ShopQuantitySpec extends FlatSpec with Matchers {

    val deserialized = ShopQuantity(UUID.fromString("bdc2a431-797d-4b07-9567-67c565a67b84"), 266)

    val serialized = "bdc2a431-797d-4b07-9567-67c565a67b84|266"

    "Transaction.apply" should "succeed to parse a valid entry" in {
        ShopQuantity(serialized) should equal (deserialized)
    }
    ...
    it should "throw an exception if a number is not formatable" in {
        a [DeserializationException] should be thrownBy ShopQuantity("5|qzefzef")
    }

    it should "throw an exception if a field is missing" in {
        a [DeserializationException] should be thrownBy ShopQuantity("5|")
    }
